...
public class PostfixToInfixAlgorithm {
        ...
        for (int i = 0; i < input.length(); i++) {
            if ("PQ10".contains(String.valueOf(input.charAt(i)))) {
                ...
                if (input.charAt(i) == '¬') {
                    ...
                    output = "(" + variables.get(variables.size() - 2) + input.charAt(i) + variables.get(variables.size() - 1) + ")";
    ...
    public static void main(String[] args) {
        ...
        System.out.println(postfixToInfix(input).replace("Λ", "a").replace("V", "o").replace("→", "i").replace("↔", "b").replace("¬", "n"));
    }
}
